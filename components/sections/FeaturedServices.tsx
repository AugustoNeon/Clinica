      ...
      <Container>
        ...
        <ul className="reveal mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            return (
              <li key={service.id} className={large ? "sm:col-span-2 lg:col-span-1 lg:row-span-2" : ""}>
                <Link
                  ...
                  className={`group relative flex h-full flex-col overflow-hidden rounded-3xl border border-ink/10 bg-surface p-6 transition duration-300 ease-out hover:-translate-y-1 hover:border-blue hover:shadow-xl hover:shadow-blue-dark/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-dark sm:p-7 ${
                    large ? "lg:p-9" : ""
                  }`}
                >
                  {large && (
                    <svg
                      aria-hidden
                      viewBox="0 0 240 120"
                      className="pointer-events-none absolute -bottom-6 -right-10 h-40 w-auto text-surface-tint transition-colors duration-300 ease-out group-hover:text-blue/20"
                      fill="none"
                    >
                  ...
                  <span
                    className={`relative inline-flex shrink-0 items-center justify-center rounded-2xl ${ICON_TINTS[index % ICON_TINTS.length]} ${
                      large ? "h-16 w-16 lg:h-20 lg:w-20" : "h-14 w-14"
                    }`}
                  >
                    <ServiceIcon slug={service.slug} className="draw-icon" width={large ? 40 : 30} height={large ? 40 : 30} />
                  </span>
                  <h3
                    className={`relative mt-6 font-medium transition-colors ease-out group-hover:text-blue-dark ${
                      large ? "text-2xl lg:text-3xl" : "text-xl sm:text-2xl"
                    }`}
                  >
                    {service.title}
                  </h3>
                  <p className={`relative mt-2 flex-1 leading-relaxed text-ink-muted ${large ? "text-base lg:text-lg" : "text-base"}`}>
                    {service.description}
                  </p>
          ...
          <li className="sm:col-span-2">
            <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-3xl bg-blue-dark p-7 text-white sm:flex-row sm:items-center sm:gap-8 sm:p-8">
